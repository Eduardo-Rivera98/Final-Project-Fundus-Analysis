...
def diagnose3(filepath,option):
    ...
    data=[['Disease', 'Result'],['Myopia', Lista[0]], ['Glaucoma', Lista[1]], ['Hypertension', Lista[2]],['Cataracts', Lista[3]], ['Diabetes', Lista[4]], ['Macular Degeneration',Lista[5]]]
    return data
...
def create_PDF(name, email, path,option):
    ...
    pdf.set_title='Eye Fundus Analysis Results'
    ...
    pdf.set_xy(42.5,60.0)
    pdf.image(path, w=60.0)
    
    # inset text comments
    pdf.set_xy(20.0,120.0)
    pdf.set_font("Arial", "U", 12)
    pdf.set_text_color(64,64,64)

    data = diagnose3(path,option)
    pdf.cell(30, 15, f"Patient name:")
    
    pdf.set_font("Arial", "", 12)
    pdf.cell(30, 15, f"{name}", ln = 1)

    pdf.set_xy(20.0,125.0)
    pdf.set_font("Arial", "U", 12)
    ...
    pdf.ln(2*th)
    # save file
    save_path = f"PDFs/{name} - {option}.pdf"
